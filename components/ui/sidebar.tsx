...
export function Sidebar({ className, children, ...props }: SidebarProps) {
  return (
    <div
      className={cn(
        "w-[200px] bg-white rounded-lg shadow-sm border border-gray-100 min-h-screen flex flex-col",
        className,
      )}
      {...props}
    >
...
export function SidebarSection({ title, className, children, ...props }: SidebarSectionProps) {
  return (
    <div className={cn("py-1", className)} {...props}>
      {title && <h3 className="px-4 text-xs font-medium text-gray-500 mb-2">{title}</h3>}
      <div>{children}</div>
    </div>
  )
}
...
export function SidebarItem({ href, icon, active, className, children, ...props }: SidebarItemProps) {
  return (
    <a
      href={href}
      className={cn(
        "flex items-center gap-3 px-4 py-2 text-sm",
        active ? "text-terreiro-green font-medium" : "text-gray-700 hover:text-terreiro-green",
        className,
      )}
      {...props}
    >
